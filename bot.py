...
for periyot_adi, aktif_mi in TARAMA_YAPILACAK_PERIYOTLAR.items():
  ...
  for ticker in tqdm(tickers, desc=f'{periyot_adi} Taranıyor'):
    try:
      ...
      min_gerekli_veri = max(CCI_PERIYOT + 5, RSI_PERIYOT + 5, 25)
      if df.empty or len(df) < min_gerekli_veri:
        continue
      ...
      avg_gain = gain.rolling(window=RSI_PERIYOT).mean()
      avg_loss = loss.rolling(window=RSI_PERIYOT).mean()
