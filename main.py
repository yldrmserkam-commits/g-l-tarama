# Gerekli kütüphanelerin sessizce yüklenmesi
...
EMA_KISA = 6
EMA_TREND = 20    # Trend Filtresi için EMA

# --- FİLTRE AKTİFLİK AYARLARI ---
HACIM_FILTRESI_AKTIF = True     # Hacim ortalamanın üzerinde olmalı
HACIM_ORT_PERIYOT = 10

TREND_FILTRESI_AKTIF = True     # Fiyat EMA 20'nin üzerinde olmalı

# Telegram Bildirim Ayarları (Yeni Token Güncellendi)
TELEGRAM_AKTIF = True
...
# Periyotların yfinance ve matematiksel parametreleri
PERIYOT_AYARLARI = {
...
for periyot_adi, aktif_mi in TARAMA_YAPILACAK_PERIYOTLAR.items():
    ...
    for ticker in tqdm(tickers, desc=f"{periyot_adi} Taranıyor"):
        try:
            ...
            # --- 3. YENİ EKLENEN FİLTRELER ---

            # A) TREND FİLTRESİ: Fiyat en azından EMA 20'nin üzerinde mi?
            if TREND_FILTRESI_AKTIF:
                if close_curr < ema20_curr:
                    continue  # Düşen trenddeki zayıf tepkileri eler

            # B) HACİM FİLTRESİ: Son mumun hacmi son 10 mumun ortalamasından büyük mü?
            if HACIM_FILTRESI_AKTIF:
                ...
                if curr_vol <= vol_sma_curr:
                    continue  # Hacimsiz sahte yükselişleri eler

            # --- TÜM FİLTRELERİ GEÇEN SİNYAL ---
            bilgi = {
...
# Sonuçları Excel'e aktarma ve indirme
if results:
